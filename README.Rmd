...
Além a aquisição e parseamento da informação, serão apresentados os tratamentos iniciais e a construção de alguns gráficos e apresentação de resultados.
...
**4)** Será apresentado um campo para selecionar uma das unidades da universidade, no exemplo, selecione **10061 - UNESP CONSOLIDADA**.
...
**5)** Novos campos serão apresentados, então, fizemos o preenchimento dos mesmos, como apresentado abaixo, para verificar os filtros disponíveis. Será gerado um relatório com *infobox*'s e gráficos, contudo, sem a tabela de dados de interesse.
...
**8)** Para entender as requisições realizadas, utilizamos a opção Inspecionar do navegador, clicando **Network**, atualizamos a página com o **F5** e ordenamos os processos por tipo (Type). Observando o tipo "document", clicamos em **ddp_tabela.php**.
...
**10)** Rolando a aba do *Header*, verificamos o parâmetros do **body** da requisição, destacados abaixo.
...
Inpecionando a página, verificamos que a unesp tem 53 unidades, assim, criamos a seguinte função para renomear as unidades, quando necessário:
...
Após o donwload, os arquivos serão apresentados como abaixo, dentro da pasta:
